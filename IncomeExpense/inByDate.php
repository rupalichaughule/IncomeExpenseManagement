...
<span style="float: left; color: green; font-weight: bold; font-size: 25px;"><?php echo $data['fname']; ?> 
  ...
  <span style="float: right; color: blue; font-weight: bold; font-size: 20px;"">
    <span>Total Income:
    <?php 
  $q=mysqli_query($conn,"select SUM(amt) from incomedata where email='".$data['email']."'") or die(mysqli_error());
  ...
  <a class="btn btn-info btn-lg" href="inByCategory.php">Search by Category</a>
  </span>
...
<br></br>
  ...
  <form align="center" class="form-inline" action="" method="post">
    <div class="form-group">
      ...
      <input type="date" class="form-control" name="inputdate1">
    </div>
    <div class="form-group">
      <label for="pwd">TO:</label>
      <input type="date" class="form-control" name="inputdate2">
    </div>
 ...
 <div class="row">
 <span style="float: left; margin-left: 350px; color: blue; font-weight: bold; font-size: 25px;">Income By Date</span>
 </div>
...
{    
  ...
  $query=mysqli_query($conn,"select * from incomedata where email='".$data['email']."' AND  indate between '{$inputdate1}' AND '{$inputdate2}' ")or die (mysqli_error($conn)); ;
...
<?php
 ...
 $query=mysqli_query($conn,"select * from incomedata where email='".$data['email']."' order by indate") or die(mysqli_error());
  while($row=mysqli_fetch_assoc($query))
  {
?>
<tr>
  ...
  <td><?php echo $row['indate'];?></td>
   ...
   <td><a class="btn btn-warning" href="incomeEdit.php?edt=<?php echo $row['id']; ?>">EDIT</a></td>
    <td><a class="btn btn-danger" href="incomeDelete.php?del=<?php echo $row['id']; ?>">DELETE</a></td>
</tr>
<?php
  }
}
